    ...
    <div id="sign">
        ...
        <div class="col-lg-6 p-0 overflow-auto" id="sign-form">
            ...
            <div class="w-100 d-flex" id="menu-sign">
                <a href="Sign_Up" type="button" class="nav-link w-50 h4 text-dark text-center">{{ __('انشاء حساب') }}</a>
                <a href="Sign" type="button" class="nav-link w-50 h4 text-dark text-center" style="background-color: var(--base-color); border-radius: 0px 0px 16px 0px;">
                    {{ __('تسجيل الدخول') }}</a>
            </div>
            <!---------------------- Sign Up ---------------------->
            <div id="signup">
                ...
                <div class="d-flex flex-column justify-content-center"
                    ...
                    <form class="d-flex flex-column" method="POST" action="{{ route('inscription.auth',app()->getLocale() ) }}"
                        id="form_signup">
                        @csrf
                        @if (session('failed'))
                        ...
                        <div class="d-flex flex-sm-row-reverse justify-content-between align-items-center">
                            ...
                            <input type="text" name="First_Name" id="first_name" placeholder="{{ __('النسب') }}"
                                class="border-0 col-form-label" style="width: 48%;">
                        </div>
                        ...
                        <input type="text" name="phone" id="phone_signup" placeholder="{{ __('رقم الهاتف') }}"
                        ...
                        <input type="password" name="conf_password" id="conf_password_signup"
                            placeholder=" {{ __('تأكيد القن السري') }}" class="border-0 col-form-label">
                        ...
                        <div class="d-flex flex-row-reverse justify-content-between align-items-center gap-3">
                            <div onsubmit="return (checkForm())" name="userForm">
                                ...
                                <input id="client" name="role" type="radio" onclick="n_identif_off()"
                                    value="">
                            </div>
                            <div class="d-flex">
                                <label for="employee" class="h6">{{ __('مستخدم') }}</label>
                                <input id="employee" name="role" type="radio" onclick="n_identif_on()"
                                    value="">
                            </div>
                            <div>
                                <input type="text" name="n_identif" id="n_identif"
                                    placeholder="{{ __('رقم التسجيل ') }}" class="border-0 col-form-label d-none"
                                    style="height: 35px;">
                            </div>
                        </div>
...
<script>
    ...
    form_signup.addEventListener('submit', (e) => {
        if ((first_name.value == "") && (last_name.value == "") && (email_signup.value == "") && (phone_signup
                .value == "") && (password_signup.value == "") && (conf_password_signup.value == "") && client
            .value == "" && employee.value == "") {
            e.preventDefault();
            error_signup.innerHTML =
            "<p class='text-danger'>{{ __('المرجوا ملأ معلوماتك لانشاء حسابك') }}</p>";
        } else {
            ...
            if ((first_name.value == "") || (last_name.value == "")) {
                ...
                if ((pattern_name.test(first_name.value)) && (first_name.value.length >= 3) && (pattern_name
                        .test(last_name.value)) && (last_name.value.length >= 3)) {
                    error_name.innerText = "";
                } else if ((!pattern_name.test(first_name.value)) || (first_name.value.length < 3) || (!
                        pattern_name.test(last_name.value)) || (last_name.value.length < 3)) {
                    e.preventDefault();
            ...
            if ((error_name.textContent == "") && (error_email_signup.textContent == "") && (error_phone_signup
                    .textContent == "") && (error_password_signup.textContent == "") && (
                    error_conf_password_signup.textContent == "") && (role_signup.textContent == "")) {
                form_signup.submit();
            }
        }
    });
